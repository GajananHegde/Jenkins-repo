...
//             sed -i "s@<% DEPLOY_AIRFLOW_DB_USER %>@${react_app_enable_micro_frontend}@g" ${environ_file} || exit 1
//             inject_env()
...
def mainfunc(String build_env, String param) {
    inject_envs()
    sh """
    sed -i "s@<% DEPLOY_AIRFLOW_DB_USER %>@${build_env}-${param}@g" ${environ_file} || exit 1
    cat ${environ_file}
    """
}